...
async def github_webhook(request: Request, db: AsyncSession = Depends(get_db)) -> None:
    ...
    sha = payload.get("after") or ((payload.get("pull_request") or {}).get("head") or {}).get("sha") or ""
    paths: list[str] = []
    ...
    snap = TerraformSnapshot(repo=repo, sha=sha, paths=paths, content=json.dumps(payload))
    db.add(snap)
    await db.commit()

    await event_bus.publish("terraform.updated", {"type": "terraform.updated", "repo": repo, "sha": sha})
