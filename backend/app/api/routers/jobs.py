...
async def scheduled_optimize(
    ...
    if not settings.scheduler_enabled:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="scheduler_disabled")

    res = await db.execute(select(TerraformSnapshot).order_by(desc(TerraformSnapshot.id)).limit(1))
    latest = res.scalar_one_or_none()
    run = OptimizationRun(repo=latest.repo if latest else "unknown", terraform_snapshot_id=latest.id if latest else None)
    db.add(run)
    await db.commit()
    await db.refresh(run)

    await event_bus.publish("run.queued", {"type": "run.queued", "repo": run.repo, "run_id": run.id})
...
async def manual_optimize(
    ...
    if body.terraform_sha:
        res = await db.execute(
            ...
            .where(TerraformSnapshot.repo == body.repo, TerraformSnapshot.sha == body.terraform_sha)
